    ...
    <section id="ubicacion" ref={ref} style={{ padding: '100px 0', background: '#FAF8F7' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 24px' }}>
        ...
        <motion.h2 initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.1 }}
          style={{ fontFamily: heading, fontSize: 'clamp(34px, 5vw, 50px)', fontWeight: 300, textAlign: 'center', color: '#111', marginBottom: 8, letterSpacing: -0.5 }}>
          Floresta, CABA
        </motion.h2>
        <motion.p initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} transition={{ duration: 0.6, delay: 0.2 }}
          style={{ fontSize: 15, color: '#7A7A7A', textAlign: 'center', marginBottom: 48, fontWeight: 300 }}>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.3 }}
          style={{ borderRadius: 18, overflow: 'hidden', border: '1px solid rgba(0,0,0,0.05)', boxShadow: '0 8px 36px rgba(0,0,0,0.05)' }}>
          <iframe
        ...
        <motion.div initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} transition={{ duration: 0.6, delay: 0.4 }}
          style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: 32, marginTop: 32, fontSize: 13, color: '#999', fontWeight: 300 }}>
